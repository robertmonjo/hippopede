...
\qquad
\gamma_0:=\gamma(x_{\max}).
}
\end{equation}
\blue{For the $k=1$ hyperconical domain one has $x_{\max}=\sqrt{3}/2$, hence}
\begin{equation}
\blue{
\gamma_0=\arctan\!\left(\frac{\sqrt{3}/2}{1/2}\right)=\arctan(\sqrt{3})=\frac{\pi}{3}.
}
\end{equation}
...
\blue{The numerical scripts used to generate the figures are included in the project folder together with the local data files required for the final four-panel comparison figure. In particular, Fig.~\ref{fig:hippopede_growth_3d} is produced by \texttt{plot\_hippopede\_growth\_3d.py}, whereas Fig.~\ref{fig:qz_final} is produced by \texttt{plot\_hippopede\_qz\_double\_panel\_with\_gapp.py}. The bundled data directory contains the curated CC/BAO compilation, the local binned Pantheon$+$ input used by the GaPP reconstruction, and the local copy of the GaPP code needed to reproduce the figure.}
...
\begin{equation}
f_{\hat r}^{\alpha}(x)=2\arctan\!\left[\frac{\gamma(x)/2}{(1-\gamma(x)/\gamma_0)^\alpha}\right]
\end{equation}
obeys
\begin{equation}
...
This result does not by itself prove that all primordial abundances are automatically reproduced, because a full BBN analysis would still require the detailed thermal history, the behaviour of $g_{*S}(T)$ and the weak-interaction freeze-out. Nevertheless, it does show that the same Monjo-type projected family already contains a physically distinguished asymptotic branch whose perceived early-time expansion scales exactly like the standard radiation era. In that sense, the pure-coasting objection to ambient-space models is softened: the low-redshift phenomenological value $\alpha\simeq0.283$ and the high-redshift physical limit $\alpha=1/2$ can be interpreted as describing two complementary regimes of the same projected construction rather than two mutually incompatible models.
...
Model dependent (BAO, CMB) and model-independent (CC, SNe Ia) [please complete and explain acronyms] produce different resulys in the estimation of the Hubble parameter, especially at the local scale (Hubble tension) but also at high-z distances (e.g. cite references). For the local expansion ($z < 0.005$?), it is obtained two very different values, the XXX from Ries et al. (year) and the XXX from YYY, with a tension up to X$\sigma$... Local void is a hypothesis, but geometrical perspectives in ambient-space constructions could help to understand the tension between model-dependent (intrinsic viewpoint) and model-independent (extrinsic viewpoint).... Acceleration and curvature are not Lorentzian invariants in cosmology, instead Ricci scalar should be used. For instance, Monjo and Campoamor-Stursberg (2023) found that ... In fact linear expansion can mimetise fictitius acceleration from coordinate transformations.
...
Finally, Gaussian ... (GaPP) approach was considered for...
...
a_\chi(t):=\rho(t,\chi)=\sqrt{\sin^2\chi+4t^2\cos^2\chi}.
\end{equation}
Its directional Hubble $H_\chi(t)= \dot a_\chi / a_\chi$ and acceleration $\ddot a_\chi(t)$ determine its deceleration function as $q_\chi(t):= - a_\chi\ddot a_\chi / \dot a_\chi^2 = - \ddot a_\chi /( a_\chi H_\chi(t)^2)$. We remark on that prescription because it yields closed analytic expressions and provides a useful first contrast with the hyperconical case with pure linear expansion (see Appendix). However, the numerical results eventually adopted for the two definitive comparison figures are based instead on a centered effective scale factor measured with respect to the center of curvature of one asymptotic tangent branch.
