...
\textcolor{red}{\sout{The same numerical exploration clarifies how the inherited anisotropy enters the projected early-time discussion. If one tries to construct a genuinely sector-dependent quantity
\begin{equation}
R_\chi(T):=\frac{H_{\chi,\mathrm{perc}}(T)}{H_{\mathrm{rad}}(T)},
\end{equation}
using the projected centered histories themselves, one finds that each fixed angle $\chi$ has only a finite internal redshift support. For the centered branches adopted in this work, the maximum unprojected redshifts are approximately
\begin{equation}
z^*_{\max}\simeq 3.0\times10^4,\ 12.36,\ 5.87,\ 3.78,\ 2.80,\ 2.13
\end{equation}
for $\chi=0^\circ$, $15^\circ$, $30^\circ$, $45^\circ$, $60^\circ$ and $85^\circ$, respectively. When these endpoints are translated back through the common projected-hyperconical map with $\alpha=1/2$, they correspond only to
\begin{equation}
z_{\mathrm{obs},\max}\simeq 422,\ 7.64,\ 5.01,\ 3.87,\ 3.23,\ 2.74,
\end{equation}
or, equivalently, to perceived temperatures of order
\begin{equation}
T_{\max}\simeq 9.9\times10^{-8},\ 2.0\times10^{-9},\ 1.4\times10^{-9},\ 1.1\times10^{-9},\ 1.0\times10^{-9},\ 8.8\times10^{-10}\ \mathrm{MeV}.
\end{equation}
These scales lie far below the standard BBN window $T\sim0.07$--$1\,\mathrm{MeV}$. Therefore the present centered lobe does not yet provide a full anisotropic BBN calculation sector by sector: the oblique projected histories simply terminate long before one reaches the redshifts relevant to primordial nucleosynthesis.

This confirms the effective observational isotropization discussed in \S\ref{sec:discussion}: the running-$\alpha$ BBN result above is an effective perceived thermal history of the projected radiation sector, not a sector-by-sector nucleosynthesis calculation on the full anisotropic background.}}\textcolor{blue}{A sector-dependent ratio $R_\chi(T):=H_{\chi,\mathrm{perc}}(T)/H_{\mathrm{rad}}(T)$ cannot be evaluated at BBN temperatures with the present centered histories: each oblique branch ($\chi\neq0^\circ$) has finite redshift support terminating far below $T\sim0.07\,\mathrm{MeV}$, so the running-$\alpha$ result above is an effective perceived thermal history of the projected axial sector only, not a sector-by-sector nucleosynthesis calculation. This confirms the effective observational isotropization discussed in \S\ref{sec:discussion}.}
