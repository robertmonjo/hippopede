...
\qquad \gamma_0 = \frac{\gamma_U}{\cos \gamma_U}
,
...
\begin{equation}
\blue{
\gamma_\text{max}=\arctan\!\left(\frac{\sqrt{3}/2}{1/2}\right)=\arctan(\sqrt{3})=\frac{\pi}{3}.
}
\end{equation}
...
\subsection{\purple{Observational Constraints on Projected Hippopede Model}}
...
\purple{We constrain $\alpha_{eff}$ using two independent low-to-intermediate redshift datasets that probe complementary redshift ranges through fundamentally different physical observables: the DES-SN5YR Type Ia supernova sample and the DESI DR1 compressed baryon acoustic oscillation measurements. The DES-SN5YR Type Ia supernova sample \cite{abbott2024dark} comprising of 1829 Type Ia supernovae in the full Hubble diagram and spanning $0.025 \le z \le 1.3$. The compressed DESI DR1 baryon acoustic oscillation catalogue \cite{adame2025desi} is the chosen intermediate redshift window, providing standard ruler measurements - $D_M/r_s, D_H/r_s,$ and  $D_V/r_s $ - across six galaxy and quasar tracer bins at effective redshifts $z_{eff}$ = $0.295$, $0.510$, $0.706$, $0.930$, $1.317$, and $1.491$. The Lyman-$\alpha$ forest bin at $z_{eff} \approx 2.33$ is excluded from the analysis, as including it mixes a significantly higher third effective redshift into the fit, and Ly-$\alpha$ BAO carry different systematic structure compared to galaxy and quasar bins \cite{}. Together the two datasets sample $\alpha_{eff}$ at two distinct effective redshifts, providing the first empirical test of its predicted redshift evolution.}
...
\begin{equation}
    \purple{\mu(z) = 5log_{10}[(1 + z_{CMB})(1 + z_{HEL})D_A(z)] + 25}
\end{equation}
...
\purple{Fitting the projected hippopede model to the DES-SN5YR sample gives $\alpha_{\mathrm{eff}} = 0.323_{-0.012}^{+0.011}$ $(1\sigma)$. The result is stable across two independent emcee runs with different stretch scale parameters, confirming convergence. Fitting to the DESI DR1 compressed BAO data gives $\alpha_{\mathrm{eff}} = 0.375_{-0.017}^{+0.016}$ $(1\sigma)$, with best-fit nuisance $\beta = 28.85$ and $\chi^2_{best} = 12.39$ for 10 data points and 2 free parameters. The flat $\Lambda$CDM with free $\Omega_m$ fitted to the same data gives $\Omega_m = 0.289$ and $\chi^2_{best} = 11.72$, corresponding to $\Delta AIC = 0.68$. The two models are statistically indistinguishable on current DESI DR1 BAO data.}

\purple{The two constraints probe distinct effective redshift ranges and are mutually consistent with the predicted slow increase of $\alpha(z)$ from the low-redshift reference $\alpha = 0.283$ at $ z \rightarrow 0$ from \cite{monjo2018projected}. An independent $H(z)$ fit using cosmic chronometers and BAO $H(z)$ points over a similar redshift range gives $\alpha_{eff} = 0.325$. Table \ref{tab:alpha_summary} summarizes the $\alpha_{eff}$ values across all constraints, including the theoretical prediction from \cite{monjo2018projected}.}
...
\caption{Alpha values over various redshift range}

\begin{tabular}{lll}
\hline
Dataset & Redshift range & $\alpha_{eff}$ \\
\hline
Low-z \cite{monjo2018projected} & $z \rightarrow 0$ & 0.283 \\
CC + BAO $H(z)$ \cite{}   &              &  0.325 \\
DES-SN5YR                 &  0.025 - 1.3 &  $0.323_{-0.012}^{+0.011}$ \\
DESI DR1 BAO              &  0.3 - 1.5   &  $0.375_{-0.017}^{+0.016}$ \\
\hline
\end{tabular}
\end{table}
...
\begin{equation}
\blue{
z_c\sim 1.6\times10^4,\qquad \Delta=2,
}
\end{equation}
...
\begin{equation}
\blue{
z_c=1.616\times10^4,\qquad \Delta=2,
}
\end{equation}
