...
\end{equation}
Since $q_\chi(t)=-\tan^2\chi/(4t^2)$, the deceleration parameter diverges to $-\infty$ for every fixed oblique direction with $\chi\neq0$ as $t\to0$\textcolor{red}{\sout{. Numerically, for $\chi=\pi/6$ one finds $q_\chi(1)=-0.0833$, $q_\chi(0.5)=-0.333$, and $q_\chi(0.1)=-8.33$, while for $\chi=\pi/4$ the corresponding values are $-0.25$, $-1$, and $-25$.}} \textcolor{blue}{(for instance, $q_{\pi/4}(0.1)=-25$).} Thus the model becomes extremely accelerated at early times unless one approaches the privileged axis $\chi=0$.\textcolor{red}{\sout{ If one wishes to compare the toy model with observationally inferred $q(z)$, the next step would be to introduce an effective isotropized scale factor $a_{\mathrm{eff}}(t)$ and then use $1+z=a_{\mathrm{eff}}(t_0)/a_{\mathrm{eff}}(t)$ to map the directional dynamics into redshift space. A dedicated fit is therefore possible, but it requires an additional averaging prescription and should probably be presented separately as a numerical extension rather than as part of the conceptual derivation given here.}}
\textcolor{red}{\sout{Hence the toy universe does not shrink to a point uniformly as $t\to0$: most angular sectors retain a finite initial radius and start with vanishing expansion rate but positive acceleration. The axis $\chi=0$ is singular in this limit, since $a_0(t)=2t$ collapses linearly to zero while the oblique sectors remain finite. This nonuniform behaviour is another sign that the model should be read as a biaxial toy geometry rather than as a realistic isotropic cosmology.}}\textcolor{blue}{The non-uniform early behaviour --- oblique sectors have finite initial radius $|\sin\chi|$ with $\dot a_\chi(0)=0$, while the axial branch collapses to zero --- confirms the biaxial rather than isotropic character of the model.}
...
For $k=1$ one has $x_{\max}=\sqrt{3}/2$ and $b(x_{\max})=1/2$. \textcolor{red}{\sout{Writing $x=x_{\max}-\delta$ with $\delta\downarrow0$, one finds
\begin{equation}
...
\end{equation}
at $T=0.07,\ 0.10,\ 0.20,\ 0.50,\ 1.0\,\mathrm{MeV}$, respectively. Relative to the BBN-inferred central history of \cite{schoneberg2024bbn}, the corresponding residuals are only $-1.2\times10^{-3},\ 4.0\times10^{-4},\ 1.0\times10^{-3},\ -1.2\times10^{-3},\ -6.3\times10^{-4}$ over the same five temperatures. \textcolor{red}{\sout{In the plotted diagnostic these values are read from the same literal projected-map evaluation after the mild log-space smoothing mentioned above, which removes small non-physical spikes produced by numerical differentiation of the dense lookup table without changing the BBN-window normalization in any material way. In other words, the same projected class admits a smooth perceived thermal history for which the whole BBN window is brought extremely close not only to the standard radiation background, but also to the currently inferred observational BBN corridor, rather than remaining strongly suppressed in the low-redshift constant-$\alpha$ case or enhanced by more than one order of magnitude in the pure $\alpha=1/2$ case.}}\textcolor{blue}{The same projected class therefore admits a smooth perceived thermal history compatible with both the standard radiation background and the observational BBN corridor, contrasting with the strongly suppressed low-$z$ constant-$\alpha$ case and the order-of-magnitude enhancement of the pure $\alpha=1/2$ case (values above are from the mildly smoothed lookup evaluation described in \S\ref{sec:methods}).}
...
The same numerical exploration clarifies how the inherited anisotropy enters the projected early-time discussion. If one tries to construct a genuinely sector-dependent quantity
\begin{equation}
