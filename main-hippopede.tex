...
\caption{
Comparison of the deceleration parameter $q(z)$ (top panels) and normalized expansion rate $E(z)=H(z)/H_0$ (bottom panels) according to the centered-scale hippopede simulations (a) and the same simulations but with stereographic projection at fixed present time $t_0=3$ (b), both for several angles ($\chi=0^\circ$. Stereographic projection $f_{\hat r}^{\alpha}$ with $\alpha \simeq 0.283$. Observational constraints shown in the upper subpanels include two present-day estimates $q_0$ from CC+Pantheon+SH0ES+BAO and $q_0=-0.50\pm0.20$ from FRB+SNe cosmography, together with two determinations of the transition redshift, $z_t=0.597\pm0.214$ and $z_t=0.64\pm0.16$. The lower subpanels also include cosmic-chronometer measurements of $H(z)$ normalized by $H_0=70\,\mathrm{km\,s^{-1}\,Mpc^{-1}}$, together with the local tangent constraints implied by the quoted $q_0$ values.}
\label{fig:qz_final}
\end{figure}
