...
\section{The coasting hippopede universe}

\subsection{Late-epoch linear expansion}
...
\blue{At the purely geometric level, the most immediate ``origin-based'' reading of the quartic hypersurface is simply $a_{\mathrm{origin},\chi}(t):=\rho(t,\chi)$. We keep that prescription below because it yields closed analytic expressions and provides a useful first contrast with the hyperconical case. However, the numerical results eventually adopted for the two definitive comparison figures are based instead on a centered effective scale factor measured with respect to the center of curvature of one asymptotic tangent branch.}

\begin{figure}[t]
\centering
\includegraphics[width=0.78\textwidth]{hippopede_growth_3d.png}
...
\subsection{Anisotropic acceleration}
\red{This subsection is useful as the analytic origin-based reference model, but it is no longer the main results section once the centered prescription is adopted for the final figures. After consolidation, it could be shortened and explicitly presented only as a contrast with the centered-scale results of Section~4.}

If one now reads $t$ as a cosmic parameter and keeps the angular sector $\chi$ fixed, the directional scale factor of this toy model is
\begin{equation}
a_\chi(t):=\rho(t,\chi)=\sqrt{\sin^2\chi+4t^2\cos^2\chi}.
\end{equation}
Its directional Hubble, acceleration and deceleration functions are then
\begin{equation}
...
\subsection{Early-epoch accelerated universe}
\red{As in the previous subsection, this origin-based early-epoch discussion may be better kept as an analytic reference or moved to an appendix after the centered results are consolidated, since the final numerical comparison figures are built from $a_{c,\chi}(t)$ rather than from $a_{\chi}(t)=\rho(t,\chi)$.}
...
\section{\blue{Methods: simulations and observational comparison}}
...
\blue{To generate the projected comparison figure, we kept the same centered histories but remapped them through the preferred projected-hyperconical construction used by Monjo in the inhomogeneous hyperconical program \cite{monjo2017hyperconical, monjo2018projected, monjo2020lagrangian, monjo2023hubble, Monjo_2024}. In the notation of \cite{monjo2018projected}, the dimensionless hyperconical radial coordinate $x$ is related to redshift through the inverse function of the line-of-sight integral}
\begin{equation}
\blue{
...
\end{equation}
be the dimensionless line-of-sight function of the $k=1$ hyperconical model. We denote its inverse function by $\xi_1^{-1}(y)$ evaluated at the logarithmic variable $y=\ln(1+z)$.
%We denote by $X_{\mathrm{from\,LZ}}(y):=\xi_1^{-1}(y)$
\blue{The preferred $\gamma$-projection of \cite{monjo2018projected} is an azimuthal locally conformal map}
\begin{equation}
\blue{
f_{\hat r}^{\alpha}(x):=
2\arctan\!\left[
\frac{\gamma(x)/2}{\left(1-\gamma(x)/\gamma_0\right)^\alpha}
\right],
}
\end{equation}
\blue{where}
\begin{equation}
\blue{
\gamma(x):=\arctan\!\left(\frac{x}{\sqrt{1-x^2}}\right),\qquad
\gamma_0:=\gamma(x_{\max}),\qquad
x(z):=\xi_1^{-1}(\ln(1+z)).
}
\end{equation}
\blue{The corresponding projected hyperconical proper distance is therefore}
\begin{equation}
\blue{
\hat r(z)= f_{\hat r}^{\alpha}\!\left(\xi_1^{-1}(\ln(1+z))\right),
}
\end{equation}
\blue{which is exactly the continuous expression implemented numerically through the preferred prescription in the results. In the numerical implementation used for the final figure, the projection parameter is fixed to $\alpha=0.283$. The projected reference Hubble function is then defined, following \cite{monjo2018projected}, by}
\begin{equation}
\blue{
...
\end{equation}
\blue{This projected figure should therefore be read as a common Monjo-type redshift remapping of the same centered hippopede simulations, not as a second independent dynamical model. Because the future continuation of the projected map is more delicate than in the unprojected case, the final projected figure is shown only for $z\geq -0.5$, whereas the unprojected one extends further into the formal future branch.}

\blue{The numerical implementation of both figures was carried out in Python from the exact scripts cited in the Data Availability statement. For the centered histories, derivatives were evaluated numerically on dense $t$-grids and the branch after the first minimum of $a_{c,\chi}(t)$ was retained whenever needed to avoid multivalued redshift histories. For the projected case, the Monjo-type redshift map was implemented numerically from the preferred $\mathrm{inv.ll}$ prescription and then used as a common remapping of the centered simulations. This should be kept in mind when interpreting the projected curves: they are numerically stable over the displayed interval, but they still depend on the chosen projection prescription and on the numerical differentiation/interpolation strategy.}

\subsection{\blue{Observed data used in the two definitive figures}}

\blue{The standard comparison curve in both figures is the flat $\Lambda$CDM$+$radiation background used in the study, with $\Omega_r=9\times10^{-5}$, $\Omega_m=0.3$, and $\Omega_\Lambda=0.69991$. The observed points used in this study are: i) the present deceleration estimate $q_0=-0.364\pm0.032$ together with the transition redshift estimate $z_t=0.597\pm0.214$ from the combined CC$+$Pantheon$+$SH0ES$+$BAO analysis of \cite{myrzakulov2025q0}; ii) the cosmographic estimate $q_0=-0.50\pm0.20$ obtained from fast radio bursts combined with supernovae in \cite{gao2024frb}; iii) the screened meta-analytic transition estimate $z_t=0.64\pm0.16$ from the latest $H(z)$ compilation analysis of \cite{hu2025transition}.}

\blue{The lower-panel $E(z)$ points come from the following cosmic-chronometer measurements, normalized in the scripts as $E_{\mathrm{obs}}(z)=H(z)/H_0$ with $H_0=70~\mathrm{km\,s^{-1}\,Mpc^{-1}}$:}
...
\caption{\blue{Observed inputs used in the lower panels of the definitive unprojected and projected centered-scale figures.}}
\label{tab:cc_data_figures}
\end{table}

\blue{In addition, the lower panel contains two local tangent constraints derived from the observed deceleration values at $z=0$. Since $q(z)=-1+(1+z)\,E'(z)/E(z)$, one has $E'(0)=1+q_0$ and therefore the two dashed tangents are represented as $E(z)\approx1+(1+q_0)z$, with the shaded band obtained from the quoted uncertainty of each $q_0$ estimate.}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{hippopede_qz_centered_t0_3p0_double_panel_with_gapp.pdf}
\caption{
Comparison of the deceleration parameter ($q(z)$) according to the centered-scale hippopede simulations (a) and the same simulations but with stereographic projection at fixed present time $t_0=3$ (b), both for several angles ($\chi=0^\circ$. Stereographic projection $f_{\hat r}^{\alpha}$ with $\alpha \simeq 0.283$. The upper subpanel displays show the deceleration parameter $q(z)$, and the lower subpanel the normalized expansion rate $E(z)=H(z)/H_0$. The centered scale factor is defined on the $+u$ branch as $a_c^2=r^2+(u-t)^2$, and the corresponding directional redshift is given by $1+z_\chi=a_{c,\chi}(t_0)/a_{c,\chi}(t)$.  Observational constraints shown in the upper subpanels include two present-day estimates of the deceleration parameter, $q_0=-0.364\pm0.032$ from CC+Pantheon+SH0ES+BAO and $q_0=-0.50\pm0.20$ from FRB+SNe cosmography, together with two determinations of the transition redshift, $z_t=0.597\pm0.214$ and $z_t=0.64\pm0.16$. The lower subpanels also include cosmic-chronometer measurements of $H(z)$ normalized by $H_0=70\,\mathrm{km\,s^{-1}\,Mpc^{-1}}$, together with the local tangent constraints implied by the quoted $q_0$ values.}
\label{fig:qz_final}
\end{figure}
...
\subsection{Scale factor centered in the tangent 3-spheres}

The origin-based prescription $a_{\mathrm{origin},\chi}(t)=\rho(t,\chi)$ remains useful because it leads to the compact analytic formulae of Sections~2.2 and~2.3. However, the final numerical comparison with $q(z)$ and $E(z)$ was built from the centered scale factor already defined in the methodological Section~3.1, because it is more directly adapted to the asymptotic picture of two tangent 3-spheres centered at $u=\pm t$. In that sense, the centered prescription is the one that better preserves the intended geometric reading of the late-time double-sphere limit.

Numerical examples computed from $a_{\mathrm{origin},\chi}(t)=\rho(t,\chi)$ are still informative as an order-of-magnitude guide. For instance, the values $q_{\pi/4}(1)=-0.25$, $q_{\pi/4}(2)=-0.0625$, and $q_{\pi/4}(10)=-0.0025$ already showed how quickly the quartic toy model tends to a coasting regime when the origin-based scale factor is used. The centered results preserve the same qualitative message than the centered geometry (i.e., with $a_{\mathrm{centered},\chi}$  ) adopted in the figures.

\subsection{\blue{Unprojected and projected expansion}}

The centered reference of the hippopede generates expansion histories that can be inspected on the same observational plane as the chronometer data and the two local tangent constraints derived from observed $q_0$ values.  The most immediate result is that they spans a broad range of behaviors between the hyperconical-like branch at $\chi=0^\circ$ and the strongly oblique sectors. The unprojected $q(z)$ curves show that the different angular sectors can interpolate between near-coasting behaviour and more strongly accelerated histories, while still remaining comparable with the observationally inferred present values and transition redshifts showed in the same panel (Fig.~\ref{fig:qz_final}). On the other hand, the common stereographic projection reparametrized the same centered histories, mapping the redshift dependence of both $q(z)$ and $E(z)$ and tends to bring the accelerating curves closer to the observations.
...
\blue{The numerical scripts used to generate the three figures currently included in the manuscript are the Python files \texttt{plot\_hippopede\_growth\_3d.py} (Fig.~\ref{fig:hippopede_growth_3d}), \texttt{plot\_hippopede\_qz\_centered\_t0\_3\_unprojected.py} (Fig.~\ref{fig:qz_unprojected_final}), and \texttt{plot\_hippopede\_qz\_centered\_t0\_3\_projected.py} (Fig.~\ref{fig:qz_projected_final}). The scripts and the corresponding PNG outputs were the exact files used to produce the figures discussed in this version of the manuscript and are available from the authors upon reasonable request.}
