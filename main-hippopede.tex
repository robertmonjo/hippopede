...
\end{equation}
Since $x_{\max} = \dots$ from the metric domain $\dots$, the value of the projecting angle is $\gamma_0 \equiv \pi/3$ [please complete]. The corresponding projected hyperconical proper distance is therefore
\begin{equation}
\blue{
\hat r(z)= f_{\hat r}^{\alpha}\!\left(\xi_1^{-1}(\ln(1+z))\right),
}
\end{equation}
\blue{which is exactly the continuous expression implemented numerically through the preferred prescription in the results. In the numerical implementation, the projection parameter is fixed to $\alpha=0.283$ according to \cite{monjo2018projected}. The projected reference Hubble function is then defined, following \cite{monjo2018projected}, by}
\begin{equation}
\blue{
