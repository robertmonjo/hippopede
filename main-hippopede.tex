...
\end{equation}
\blue{with $\alpha_{\mathrm{low}}=0.283$ fixed by the successful low-redshift phenomenology of Fig.~\ref{fig:qz_final}, and $\alpha_{\mathrm{high}}=1/2$ fixed by the radiation-like asymptotic requirement. Exploratory scans with the supplementary script \texttt{analyze\_hippopede\_variable\_alpha.py} and the direct projected-map diagnostics show that, in the literal numerical implementation, the most promising BBN-like window is obtained not for a very late transition but for a much earlier and narrower one, approximately around}
\begin{equation}
\blue{
z_c\sim 2\times10^4,\qquad \Delta\sim 2,
}
\end{equation}
\blue{yields an apparent BBN normalization much closer to the standard radiation-era value while preserving the low-redshift sector almost unchanged. A representative near-optimal case is}
\begin{equation}
\blue{
...
0.970,\ 0.983,\ 0.966,\ 1.018,\ 1.030
}
\end{equation}
\blue{at $T=0.07,\ 0.10,\ 0.20,\ 0.50,\ 1.0\,\mathrm{MeV}$, respectively. In other words, the same projected family admits a smooth perceived thermal history for which the whole BBN window is brought extremely close to the standard radiation background in the literal projected-map calculation, rather than remaining separated by four orders of magnitude as in the low-redshift constant-$\alpha$ branch or by a factor $\sim10$ as in the pure $\alpha=1/2$ branch.}

\blue{This does not yet amount to a full light-element calculation, and we therefore do not claim in this paper that the primordial abundances are already reproduced in detail. Nevertheless, the numerical result is strong enough to support a more focused statement: within the projected-hyperconical interpretation, the apparent thermal history can be made quantitatively compatible with the order-of-magnitude normalization expected for a BBN-like radiation era, while still approaching the asymptotic $T^2$ scaling singled out by the high-redshift argument, provided one allows a smooth transition from the low-redshift phenomenological value $\alpha\simeq0.283$ to the asymptotic physical value $\alpha=1/2$. A complete BBN claim would then require the next layer of analysis, namely a consistent treatment of $g_{*S}(T)$, the weak-interaction freeze-out, and the light-element reaction network.}
