...
\begin{equation}
\blue{
z_c\sim 2\times10^4,\qquad \Delta\sim 2,
}
\end{equation}
\blue{This choice preserves the low-redshift sector almost unchanged, since $\alpha(1)\simeq0.28302$ and $\alpha(2)\simeq0.28303$, while driving the projected thermal history very close to the standard radiation-era normalization across the BBN window. A representative near-optimal case is}
\begin{equation}
\blue{
z_c=2.056\times10^4,\qquad \Delta=1.95,
}
\end{equation}
\blue{for which the geometric-mean normalization over the interval $T\simeq0.07$--$1\,\mathrm{MeV}$ is}
\begin{equation}
\blue{
...
0.969,\ 0.980,\ 0.997,\ 1.021,\ 1.037
}
\end{equation}
\blue{at $T=0.07,\ 0.10,\ 0.20,\ 0.50,\ 1.0\,\mathrm{MeV}$, respectively. In the plotted diagnostic these values are read from the same literal projected-map evaluation after the mild log-space smoothing mentioned above, which removes small non-physical spikes produced by numerical differentiation of the dense lookup table without changing the BBN-window normalization in any material way. In other words, the same projected family admits a smooth perceived thermal history for which the whole BBN window is brought extremely close to the standard radiation background in the literal projected-map calculation, rather than remaining strongly suppressed in the low-redshift constant-$\alpha$ branch or enhanced by more than one order of magnitude in the pure $\alpha=1/2$ branch.}
